...
from kodiak.core.error_handling import (
    ErrorHandler, WebSocketError, handle_errors, ErrorCategory
)
...
class ConnectionManager:
    ...
    @handle_errors(ErrorCategory.WEBSOCKET, reraise=False)
    async def connect(self, websocket: WebSocket, scan_id: str = None):
        try:
            ...
            raise ErrorHandler.handle_websocket_error(
                error=e,
                client_id=scan_id or "global",
    ...
    def disconnect(self, websocket: WebSocket, scan_id: str = None):
        try:
            ...
            # Log but don't raise - disconnection should always succeed
            ErrorHandler.handle_websocket_error(
                error=e,
                client_id=scan_id or "global",
    ...
    async def broadcast(self, scan_id: str, message: dict):
        ...
        for connection in self.active_connections[scan_id]:
            try:
                ...
                # Handle the error but don't stop broadcasting to other clients
                ErrorHandler.handle_websocket_error(
                    error=e,
                    client_id=scan_id,
                    context={
        ...
        if disconnected:
            logger.info(f"Broadcast to scan {scan_id}: {successful_sends} successful, {len(disconnected)} failed")
    ...
    async def broadcast_global(self, message: dict):
        ...
        # Log broadcast statistics
        if disconnected:
            logger.info(f"Global broadcast: {successful_sends} successful, {len(disconnected)} failed")
    ...
    @handle_errors(ErrorCategory.WEBSOCKET, reraise=False)
    async def send_error_notification(self, scan_id: str, error: Dict[str, Any]):
